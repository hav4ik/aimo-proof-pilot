...
RUN_NAME="${OLMO_RUN_DIR_NAME:-prime_rl_opd_muon_colocated_tp2_testcsv_$(date -u +%Y%m%d_%H%M%S)}"
...
/usr/bin/python /app/train.py \
  ...
  --max_grad_norm 1.0 \
  --prime_algorithm opd \
  --prime_opd_teacher_model /vol/olmo_train_assets/models/opd-32b-deploy/opd-32b-deploy \
  --prime_opd_start_teacher true \
  --prime_opd_teacher_gpu_ids 0,1 \
  --prime_opd_teacher_port 8001 \
  --prime_opd_teacher_vllm_tensor_parallel_size 2 \
  ...
  --prime_opd_teacher_vllm_gpu_memory_utilization 0.4 \
  --prime_opd_teacher_vllm_max_num_seqs 4 \
  --prime_opd_teacher_vllm_max_num_batched_tokens 16384 \
