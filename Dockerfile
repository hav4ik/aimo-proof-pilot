...
# Install the daemon deps (gradio_client) into the SYSTEM python. No venv: a uv-managed venv python lives
# under /root/.local, which a non-root user (e.g. on NII/Singularity) can't execute -> "permission denied".
# gradio_client's deps are already provided by the training stack, so this adds only gradio_client itself.
